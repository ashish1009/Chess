...
bool Chess::MoveQueen(unsigned short dstX, unsigned short dstY)
{
    ...
    m_bDestinationValidity = ((dstY == src.Y) ? MoveStraight(dstX, src.Y, src.X, 'X') : ((dstX == src.X) ? MoveStraight(dstY, src.X, src.Y, 'Y') : MoveDiagonal(dstY, dstX, src.Y, src.X)));
